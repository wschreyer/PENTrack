...
void TXenon::OnHit(TStep &stepper, const double normal[3],
...
void TXenon::OnStep(TStep &stepper,
		const solid &currentsolid, TMCGenerator &mc, stopID &ID, std::vector<TParticle*> &secondaries) const{

}

//Xenon does not decay
void TXenon::Decay(const TStep &stepper, TMCGenerator &mc, const TGeometry &geom, const TFieldManager &field, std::vector<TParticle*> &secondaries) const{
