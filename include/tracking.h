...
private:
  ...
  TLogger* logger; ///< class to log particle states // sly
...
private:
  ...
  bool CheckHit(const std::unique_ptr<TParticle>& p, const value_type x1, const state_type &y1, value_type &x2, state_type &y2,
		const dense_stepper_type &stepper, TMCGenerator &mc, const TGeometry &geom, const TFieldManager &field, std::vector<std::pair<solid, bool> > &currentsolids);
  ...
  bool DoHit(const std::unique_ptr<TParticle>& p, const value_type x1, const state_type &y1, value_type &x2, state_type &y2,
	     const dense_stepper_type &stepper, TMCGenerator &mc, const TGeometry &geom, std::vector<std::pair<solid, bool> > &currentsolids);

  /**
   * Return first non-ignored solid in TParticle::currentsolids list
   */
  // const solid& GetCurrentsolid() const;
  const solid& GetCurrentsolid(std::vector<std::pair<solid, bool> > currentsolids) const;
  /**
