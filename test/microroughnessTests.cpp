...
BOOST_AUTO_TEST_CASE(microroughnessDistributionTest){
    material vacuumMat = {"vacuum", 0., 0., 0., 0., 0., 0., 0., 0., 0.};
    // parameters for Cu surface from Steyerl's microroughness paper (10.1007/BF01380066)
    double kCu = 0.00894/1e-10; // critical wave number
    material copperMat = {"copper", static_cast<double>(hbar*hbar*kCu*kCu/2./m_n/ele_e/ele_e*1e9), 0., 0., 0., 35e-10, 250e-10, 0., 0., 0.};
    ...
    solid vacuum = {"", "vacuum", vacuumMat, 1, {}};
    solid copper = {"", "copper", copperMat, 2, {}};
    for (auto isoLine: steyerlCopperReflectionData){
        for (auto dataPoint: isoLine.second){
            BOOST_TEST_CONTEXT("Parameters: theta = " << dataPoint.first << ", phi = " << dataPoint.second << ", MRprob = " << isoLine.first){
                double MRreflection = MR::MRDist(false, false, &v[0], &normal[0], vacuum, copper, dataPoint.first/180.*pi, dataPoint.second/180.*pi);
    ...
    for (auto isoLine: steyerlCopperTransmissionData){
        for (auto dataPoint: isoLine.second){
            BOOST_TEST_CONTEXT("Parameters: theta = " << dataPoint.first << ", phi = " << dataPoint.second << ", MRprob = " << isoLine.first){
                double MRtransmission = MR::MRDist(true, false, &v[0], &normal[0], vacuum, copper, dataPoint.first/180.*pi, dataPoint.second/180.*pi);
    ...
    for (auto dataPoint: steyerlTotalReflectionData){
        ...
        double copperReflection = MR::MRProb(false, &v[0], &normal[0], vacuum, copper);
        double copperTransmission = MR::MRProb(true, &v[0], &normal[0], vacuum, copper);
    ...
    std::cout << "Calculated " << 2*steyerlTotalReflectionData.size() << " microroughness integrals in " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << " us\n";

/*
    material nickelMat = {"nickel", 230., 0., 0., 0., 35e-10, 250e-10};
    double kNi = sqrt(2.*m_n*nickelMat.FermiReal*1e-9)*ele_e/hbar;
    solid nickel = {"", "nickel", nickelMat, 3, {}};

    if (k > kNi) theta_i = acos(2./3.*kNi/k);
    else theta_i = 48./180.*pi;
    v = {vabs*sin(theta_i), 0., vabs*cos(theta_i)};
    double nickelReflection = 0, nickelTransmission = 0;
    nickelReflection = MR::MRProb(false, &v[0], &normal[0], vacuum, nickel);
    nickelTransmission = MR::MRProb(true, &v[0], &normal[0], vacuum, nickel);
*/
}