      ...
      for (kk = 0; kk < kN - kM; kk++)
        {
          ...
          mt[kk] = mt[kk + kM] ^ (y >> 1) ^ MAGIC(y);
        }
      for (; kk < kN - 1; kk++)
        {
          unsigned long y = (mt[kk] & UPPER_MASK) | (mt[kk + 1] & LOWER_MASK);
          mt[kk] = mt[kk + (kM - kN)] ^ (y >> 1) ^ MAGIC(y);
        }

      {
        unsigned long y = (mt[kN - 1] & UPPER_MASK) | (mt[0] & LOWER_MASK);
        mt[kN - 1] = mt[kM - 1] ^ (y >> 1) ^ MAGIC(y);
      }
...
// return val is double and of intervall 0,1
 long double mt_get_double (void * vstate)
{
  return (long double) mt_get (vstate) / 4294967296.0 ;
}
